...
export default function ExperienceShowcase({ initialData }: ExperienceShowcaseProps) {
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(true);
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  const { data: experiences = [] } = useQuery<ExperienceCardDto[]>({
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-lg text-gray-700/80">
            Authentic Sri Lankan experiences crafted to immerse you into our rich culture.
          </p>
        </div>
          ...
          <div ref={scrollContainerRef} className="flex overflow-x-auto gap-6 pb-4 snap-x snap-mandatory scrollbar-hide" onScroll={checkScrollable}>
            {experiences.map((experience: ExperienceCardDto) => (
              <div key={experience.id} className="flex-none w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] snap-start">
                <div className="bg-[#F8F5F0] rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group border border-gray-100">
